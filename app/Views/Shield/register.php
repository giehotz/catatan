...
<div class="max-w-md mx-auto my-12 bg-surface/60 p-8 rounded-2xl border border-br-default shadow-2xl relative overflow-hidden">
    ...
    <div class="space-y-6">
        <div class="text-center space-y-2">
            <h2 class="text-3xl font-extrabold text-tx-primary tracking-tight">Daftar Akun Baru</h2>
            <p class="text-tx-secondary text-sm">Mulai kelola catatan keuangan Anda secara profesional.</p>
        </div>
        ...
        <form action="<?= url_to('register') ?>" method="post" class="space-y-4">
            ...
            <div class="space-y-1.5">
                <label for="floatingUsernameInput" class="text-xs font-bold text-tx-secondary uppercase tracking-wider">Nama Pengguna (Username)</label>
                <input type="text" class="w-full px-4 py-3 bg-base/60 border border-br-default rounded-xl focus:border-brand focus:ring-1 focus:ring-brand text-tx-primary placeholder-tx-disabled/50 transition-all outline-none" id="floatingUsernameInput" name="username" inputmode="text" placeholder="username123" value="<?= old('username') ?>" required>
            </div>
            ...
            <!-- Password Confirm -->
            <div class="space-y-1.5">
                <label for="floatingPasswordConfirmInput" class="text-xs font-bold text-tx-secondary uppercase tracking-wider">Konfirmasi Kata Sandi</label>
                <div class="relative">
                    <input type="password" class="w-full pl-4 pr-12 py-3 bg-base/60 border border-br-default rounded-xl focus:border-brand focus:ring-1 focus:ring-brand text-tx-primary placeholder-tx-disabled/50 transition-all outline-none" id="floatingPasswordConfirmInput" name="password_confirm" inputmode="text" placeholder="Masukkan kembali kata sandi" required>
                    <button type="button" onclick="togglePasswordVisibility('floatingPasswordConfirmInput', this)" class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-tx-secondary hover:text-brand transition-colors cursor-pointer" title="Tampilkan kata sandi">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                        </svg>
        ...
        <div class="text-center pt-2">
            <p class="text-sm text-tx-secondary">
                Sudah memiliki akun? 
                <a href="<?= url_to('login') ?>" class="text-brand hover:text-brand-hover font-semibold transition-colors">Masuk di sini</a>
            </p>
        </div>
    </div>
</div>
