...
<div class="space-y-8 max-w-6xl mx-auto py-4 mt-4 animate-fade-in">
    ...
    <div class="flex items-center justify-between">
        ...
        <span class="text-xs text-slate-500 font-semibold font-mono">Billing & Ledgers</span>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div class="bg-slate-950/40 border border-slate-900 rounded-2xl p-5 relative overflow-hidden backdrop-blur-sm">
            <div class="absolute top-0 right-0 w-24 h-24 bg-rose-500/5 rounded-full blur-2xl pointer-events-none"></div>
            <div class="flex items-center gap-4">
                <div class="p-3 rounded-xl bg-slate-900 border border-slate-800 text-rose-400 shadow-inner">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                </div>
                <div>
                    <span class="text-[10px] text-slate-500 uppercase tracking-widest font-bold block">Tagihan Belum Lunas</span>
                    <strong class="text-2xl font-extrabold text-white tracking-tight block mt-0.5"><?= $unpaidCount ?> Tagihan</strong>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-950/40 border border-slate-900 rounded-2xl p-5 relative overflow-hidden backdrop-blur-sm">
            ...
            <div class="flex items-center gap-4">
                <div class="p-3 rounded-xl bg-slate-900 border border-slate-800 text-indigo-400 shadow-inner">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div>
                    <span class="text-[10px] text-slate-500 uppercase tracking-widest font-bold block">Total Tunggakan Iuran</span>
        ...
        <div class="bg-slate-950/40 border border-slate-900 rounded-2xl p-5 relative overflow-hidden backdrop-blur-sm flex flex-col justify-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-slate-900/40 border border-slate-900 rounded-2xl overflow-hidden shadow-xl">
            <div class="p-6 border-b border-slate-900 flex items-center justify-between">
                <div>
                    <h3 class="text-base font-bold text-white tracking-tight">Status Simpanan Bulanan Saya</h3>
                    <p class="text-[11px] text-slate-500">Daftar iuran simpanan wajib & dana sosial sejak bergabung.</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left text-slate-300">
                    <thead class="bg-slate-950/40 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-900">
                        <tr>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-between gap-3">
                                        <div>
                                            ...
                                            <?php else : ?>
                                                <span class="px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-slate-800 text-slate-400 border border-slate-700 uppercase">Belum Bayar</span>
                                            <?php endif; ?>
                                        </div>
                                        <?php if ($b['wajib_status'] === 'Belum Bayar' || $b['wajib_status'] === 'Menunggak') : ?>
                                            <button onclick="openPaymentModal('wajib', <?= $b['bulan'] ?>, <?= $b['tahun'] ?>, <?= $wajibNominal ?>)" class="cursor-pointer px-2 py-1 rounded bg-indigo-600/15 hover:bg-indigo-600/30 text-indigo-400 hover:text-white border border-indigo-500/20 font-bold transition-all text-[10px]">
                                                Bayar
                                            </button>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-between gap-3">
                                        ...
                                        <?php if ($b['sosial_status'] === 'Belum Bayar' || $b['sosial_status'] === 'Menunggak') : ?>
                                            <button onclick="openPaymentModal('sosial', <?= $b['bulan'] ?>, <?= $b['tahun'] ?>, <?= $sosialNominal ?>)" class="cursor-pointer px-2 py-1 rounded bg-indigo-600/15 hover:bg-indigo-600/30 text-indigo-400 hover:text-white border border-indigo-500/20 font-bold transition-all text-[10px]">
                                                Bayar
                                            </button>
                                        <?php endif; ?>
                                    </div>
        ...
        <div class="space-y-6 lg:col-span-1">
            <div class="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 space-y-6">
                <div>
                    <h3 class="text-base font-bold text-white tracking-tight">Jadwal Angsuran Aktif</h3>
                    <p class="text-[11px] text-slate-500">Amortisasi pinjaman yang sedang berjalan.</p>
                </div>

                <?php if (empty($activeLoans)) : ?>
                    <div class="p-6 text-center text-slate-500 font-semibold border border-dashed border-slate-800 rounded-xl text-xs">
                        Tidak ada fasilitas pinjaman aktif saat ini.
                    </div>
                <?php else : ?>
                    <?php foreach ($activeLoans as $loan) : ?>
                        <div class="space-y-4">
                            <div class="p-4 bg-slate-950/60 border border-slate-900/60 rounded-xl space-y-2 text-xs">
                                <div class="flex justify-between text-slate-400">
                                    <span>No Pinjaman:</span>
                                    <strong class="text-white">#<?= $loan['id'] ?></strong>
                                </div>
                                <div class="flex justify-between text-slate-400">
                                    <span>Pokok Awal:</span>
                                    <strong class="text-white">Rp <?= number_format($loan['nominal_pinjaman'], 0, ',', '.') ?></strong>
                                </div>
                                <div class="flex justify-between text-slate-400">
                                    <span>Tenor:</span>
                                    <strong class="text-white"><?= $loan['tenor_bulan'] ?> Bulan</strong>
                                </div>
                                <div class="flex justify-between text-slate-400 border-t border-slate-900/60 pt-2 font-bold text-indigo-400">
                            ...
                            <div class="space-y-2">
                                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Daftar Angsuran</span>
                                <div class="space-y-2 max-h-72 overflow-y-auto no-scrollbar border border-slate-900 rounded-xl p-2.5 bg-slate-950/20">
                                    <?php foreach ($loan['amortization'] as $am) : ?>
                                        <div class="flex items-center justify-between text-xs p-2 rounded-lg bg-slate-900/30 border border-slate-900">
                                            <div class="space-y-0.5">
                                                <strong class="text-slate-200">Cicilan Ke-<?= $am['angsuran_ke'] ?></strong>
                                                <span class="text-[9px] text-slate-500 block">Jatuh Tempo: <?= $am['due_date'] ?></span>
                                            </div>
                                            <div class="text-right">
                                                <strong class="text-white block text-[11px]">Rp <?= number_format($am['total'], 0, ',', '.') ?></strong>
...
<div id="paymentModal" class="fixed inset-0 z-50 hidden bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative animate-scale-up">
        ...
        <div class="p-6 border-b border-slate-800 flex items-center justify-between">
            ...
            <button onclick="closePaymentModal()" class="text-slate-500 hover:text-white cursor-pointer transition-colors">
        ...
        <form action="<?= base_url('cooperative/pay-saving-bill') ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-4 text-xs">
            ...
            <div class="p-4 bg-slate-950/60 rounded-xl space-y-1.5 text-slate-400 border border-slate-900">
                <div class="flex justify-between">
                    <span>Jenis Tagihan:</span>
                    <strong id="label_jenis" class="text-white"></strong>
                </div>
                <div class="flex justify-between">
                    <span>Bulan / Tahun:</span>
                    <strong id="label_periode" class="text-white"></strong>
                </div>
                <div class="flex justify-between border-t border-slate-900/60 pt-2 font-bold text-indigo-400">
            ...
            <div class="space-y-1.5">
                <label for="bukti_transfer" class="text-xs font-bold text-slate-400 uppercase tracking-wider block">Unggah Bukti Transfer (Image)</label>
                <input type="file" id="bukti_transfer" name="bukti_transfer" required accept="image/*" class="w-full px-3 py-2 bg-slate-950 border border-slate-900 rounded-lg text-slate-300 outline-none file:mr-3 file:py-1 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-bold file:bg-indigo-600/15 file:text-indigo-400 hover:file:bg-indigo-600/30 cursor-pointer">
            </div>

            <button type="submit" class="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-lg transition-all cursor-pointer">
...
<div id="annualModal" class="fixed inset-0 z-50 hidden bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative animate-scale-up">
        ...
        <div class="p-6 border-b border-slate-800 flex items-center justify-between">
            ...
            <button onclick="closeAnnualModal()" class="text-slate-500 hover:text-white cursor-pointer transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
        ...
        <form action="<?= base_url('cooperative/pay-saving-bill') ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-4 text-xs">
            ...
            <div class="space-y-1.5">
                <label for="annual_tahun" class="text-xs font-bold text-slate-400 uppercase tracking-wider block">Pilih Tahun Pembayaran</label>
                <select id="annual_tahun" name="tahun" class="w-full px-4 py-3 bg-slate-950 border border-slate-900 rounded-xl text-white font-bold outline-none cursor-pointer focus:border-emerald-500">
                    <option value="<?= date('Y') ?>" selected>Tahun <?= date('Y') ?></option>
                    <option value="<?= date('Y') + 1 ?>">Tahun <?= date('Y') + 1 ?></option>
                </select>
            </div>

            <div class="p-4 bg-slate-950/60 rounded-xl space-y-2 text-slate-400 border border-slate-900">
                <div class="flex justify-between">
                    <span>Simpanan Wajib (12 Bln):</span>
                    <strong class="text-white">Rp <?= number_format($wajibNominal * 12, 0, ',', '.') ?></strong>
                </div>
                <div class="flex justify-between">
                    <span>Dana Sosial (12 Bln):</span>
                    <strong class="text-white">Rp <?= number_format($sosialNominal * 12, 0, ',', '.') ?></strong>
                </div>
                <div class="flex justify-between border-t border-slate-900/60 pt-2 font-bold text-emerald-400">
            ...
            <div class="space-y-1.5">
                <label for="annual_bukti" class="text-xs font-bold text-slate-400 uppercase tracking-wider block">Unggah Bukti Transfer (Image)</label>
                <input type="file" id="annual_bukti" name="bukti_transfer" required accept="image/*" class="w-full px-3 py-2 bg-slate-950 border border-slate-900 rounded-lg text-slate-300 outline-none file:mr-3 file:py-1 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-bold file:bg-indigo-600/15 file:text-indigo-400 hover:file:bg-indigo-600/30 cursor-pointer">
            </div>

            <button type="submit" class="w-full py-3 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold rounded-xl shadow-lg transition-all cursor-pointer">
