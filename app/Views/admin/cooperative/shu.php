 ...
 * @var int $tahun
 * @var float $totalShu
 * @var float $jasaModalPercent
 * @var float $jasaAnggotaPercent
 * @var array $simulation
 * @var array $shuHistory
 * @var float $totalCoopSavings
 * @var float $totalCoopInterest
 */
?>
...
<div class="space-y-8 mt-4">
    ...
    <div class="space-y-1">
        ...
        <p class="text-slate-400 text-sm">Simulasikan pembagian Sisa Hasil Usaha (SHU) berdasarkan kontribusi modal dan pinjaman, serta bagikan secara langsung ke Simpanan Sukarela anggota.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 shadow-xl lg:col-span-1">
            ...
            <form action="<?= base_url('admin/cooperative/shu') ?>" method="GET" class="space-y-4">
                ...
                <div>
                    <label for="total_shu" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Total SHU Dibagi (Rp)</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="jasa_modal_percent" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Jasa Modal (%)</label>
                        <input type="number" id="jasa_modal_percent" name="jasa_modal_percent" min="0" max="100" value="<?= (int)$jasaModalPercent ?>" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-hidden focus:ring-1 focus:ring-indigo-500/50" oninput="document.getElementById('jasa_anggota_percent').value = 100 - this.value">
                    </div>
                    <div>
                        <label for="jasa_anggota_percent" class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Jasa Anggota (%)</label>
                        <input type="number" id="jasa_anggota_percent" name="jasa_anggota_percent" min="0" max="100" value="<?= (int)$jasaAnggotaPercent ?>" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-hidden focus:ring-1 focus:ring-indigo-500/50" oninput="document.getElementById('jasa_modal_percent').value = 100 - this.value">
                    </div>
                </div>
        ...
        <!-- Global Cooperative Statistics Summary -->
        <div class="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 shadow-xl lg:col-span-2 flex flex-col justify-between">
            <div>
                <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    ...
                    Ringkasan Konstanta Koperasi (Aktif)
                </h3>
                <p class="text-slate-400 text-xs mb-4">Total simpanan dan bunga kredit yang digunakan sebagai penyebut dalam pembagian rasio pembagian SHU.</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="p-4 bg-slate-950/60 rounded-xl border border-slate-900">
                        <span class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Total Pendapatan Bunga Kredit</span>
                        <span class="text-xl font-bold text-emerald-400 font-mono">Rp <?= number_format($totalCoopInterest, 2, ',', '.') ?></span>
                    </div>
                </div>
            </div>
            ...
            <?php if ($totalShu > 0 && count($simulation) > 0) : ?>
                <div class="mt-6 p-4 bg-indigo-500/5 border border-indigo-500/10 rounded-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-slate-400 text-[11px] leading-relaxed">Kalkulasi simulasi di bawah ini akan disimpan ke histori dan saldo simpanan sukarela anggota akan langsung bertambah secara real-time.</p>
                    </div>
                    ...
                    <form action="<?= base_url('admin/cooperative/shu/distribute') ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membagikan total SHU sebesar Rp <?= number_format($totalShu, 0, ',', '.') ?> ke semua anggota aktif? Tindakan ini akan langsung mengkreditkan dana ke Simpanan Sukarela mereka.')">
                        ...
                        <input type="hidden" name="jasa_modal_percent" value="<?= $jasaModalPercent ?>">
                        <input type="hidden" name="jasa_anggota_percent" value="<?= $jasaAnggotaPercent ?>">
                ...
                <table class="w-full text-left text-sm text-slate-300">
                    <thead class="bg-slate-950/80 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-900">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-right">Partisipasi Modal</th>
                            <th class="px-6 py-4 text-right">Partisipasi Usaha</th>
                            <th class="px-6 py-4 text-right text-indigo-400">Jasa Modal</th>
                            <th class="px-6 py-4 text-right text-emerald-400">Jasa Anggota</th>
                        ...
                        <?php else : ?>
                            <?php foreach ($simulation as $sim) : ?>
                                <tr class="hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-right font-mono text-xs">Rp <?= number_format($sim['total_interest'], 2, ',', '.') ?></td>
                                    <td class="px-6 py-4 text-right font-mono text-xs text-indigo-400">Rp <?= number_format($sim['jasa_modal'], 2, ',', '.') ?></td>
                                    <td class="px-6 py-4 text-right font-mono text-xs text-emerald-400">Rp <?= number_format($sim['jasa_anggota'], 2, ',', '.') ?></td>
            ...
            <table class="w-full text-left text-sm text-slate-300">
                <thead class="bg-slate-950/80 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-900">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-right">Jasa Anggota</th>
                    ...
                    <?php else : ?>
                        <?php foreach ($shuHistory as $history) : ?>
                            <tr class="hover:bg-slate-800/20 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right font-mono text-xs">Rp <?= number_format($history['jasa_anggota'], 2, ',', '.') ?></td>
